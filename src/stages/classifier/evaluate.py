...
from ...paths import (
    ...
    CLS_LAST_METRICS, CLS_BEST_METRICS,
...
def evaluate_classifier(model, is_better, val_emb=None, label_encoder_path=None):
    ...
    CLS_LAST_METRICS.mkdir(parents=True, exist_ok=True)
    save_json(CLS_LAST_METRICS / "val_metrics.json", metrics)

    supercat_names = list(
        build_supercategory_name_mapping(load_json(SUPERCATEGORIES_PATH)).values()
    )

    # visualization imports deferred to avoid circular deps before visualization/ is ready
    ...
    plot_confusion_matrix(
        ...
        save_path=CLS_LAST_METRICS / "confusion_matrix.png",
    )

    show_classifier_predictions(
        model, label_encoder, config.device,
        save_dir=CLS_LAST_METRICS,
    )

    if is_better:
        CLS_BEST_METRICS.mkdir(parents=True, exist_ok=True)
        shutil.copy(CLS_LAST_METRICS / "val_metrics.json",    CLS_BEST_METRICS / "val_metrics.json")
        shutil.copy(CLS_LAST_METRICS / "confusion_matrix.png", CLS_BEST_METRICS / "confusion_matrix.png")
