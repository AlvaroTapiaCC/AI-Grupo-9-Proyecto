...
def train_classifier(
    ...
    for d in (CLS_LAST_MODEL, CLS_BEST_MODEL, CLS_LAST_METRICS, CLS_BEST_METRICS):
    ...
    save_model(model, CLS_LAST_MODEL / "model.pt")

    y_true, y_pred = get_classifier_predictions(model, test_loader, config.device)
    metrics        = compute_classifier_metrics(y_true, y_pred)

    save_json(CLS_LAST_MODEL   / "history.json", history)
    save_json(CLS_LAST_METRICS / "metrics.json", metrics)

    best_metrics_file = CLS_BEST_METRICS / "metrics.json"
    is_better = True
    ...
    if is_better:
        ...
        save_model(model, CLS_BEST_MODEL / "model.pt")
        shutil.copy(CLS_LAST_MODEL   / "history.json", CLS_BEST_MODEL   / "history.json")
        shutil.copy(CLS_LAST_METRICS / "metrics.json", CLS_BEST_METRICS / "metrics.json")
